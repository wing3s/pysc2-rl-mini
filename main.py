...
parser.add_argument(
    '--map-name',
    default='FindAndDefeatZerglings',
    metavar='MAP',
...
def main():
    ...
    for idx in range(args.num_processes):
        actor_thread = mp.Process(
            target=actor_fn, args=(idx, args, shared_model, optimizer))
        actor_thread.start()
        processes.append(actor_thread)

    # start a thread for policy evaluation
    monitor_thread = mp.Process(
        target=monitor_fn, args=(args.num_processes, args, shared_model))
